...
Implementations of the base classes in Transforms.py, covering SO(2), SO(3), and SE(3) with utility functions
...
from robotics_src.Transforms import SO2 as SO2Base
from robotics_src.Transforms import SO3 as SO3Base
from robotics_src.Transforms import SE3 as SE3Base
...
class SO2(SO2Base):
    ...
    """
    def __init__(self, angle, radians=True):
        """ 
        R = SO2(angle, radians=True)
        Parameters:
...
class SO3(SO3Base):
    ...
    def rpy(self):
        ...
        Returns:
        pose - 1 x 3 Sympy Matrix, containing <roll pitch yaw> coordinates (in radians)
        """
        # follow formula in book, see equation (2.22) on pg. 52
    ...
    def axis(self):
        ...
        Returns:
        pose - 1 x 4 sympy matrix, containing  the axis angle representation
    ...
    def quaternion(self):
        ...
        Returns:
        pose - 1 x 4 sympy matrix, quaternion representation of pose in the 
        format <nu, ex, ey, ez>
        """
...
def rotx(theta, radians=True):
    """
    R = rotx(theta, radians=True)
    Description:
...
def euler2R(th1, th2, th3, order='xyx', radians=True):
    ...
    Description:
    Returns an SO3 object of the rotation specified by the euler angles
...
def trotx(theta, radians=True):
    """
    A = trotx(theta, radians=True)
    Description:
    Creates an SE3 object by defining a rotation
    ...
    A - SE3 object
    """
    R = rotx(theta, radians)
    return SE3(R=R)

def troty(theta, radians=True):
    """
    A = trotx(theta, radians=True)
    Description:
    Creates an SE3 object by defining a rotation
    ...
    A - SE3 object
    """
    R = roty(theta, radians)
    return SE3(R=R)

def trotz(theta, radians=True):
    """
    A = trotx(theta, radians=True)
    Description:
    Creates an SE3 object by defining a rotation
...
def translate(p):
    ...
    A = translate(p)
    Description:
    Returns an SE3 object based on a translation p

    Parameters:
    p - 3x1 sympy matrix representing 3D translation or position
...
def q2R(q):
    ...
    R = q2R(q)
    Description:
    Returns a 3x3 sympy matrix representing rotation generated from a quaternion q

    Args:
        q: 4x1 sympy matrix or length 4 iterable representing a quaternion

    Returns:
        R: 3x3 sympy matrix, representing the rotation
    """
    return q

def R2axis(R):
    ang = sp.acos(0.5 * (R[0, 0] + R[1, 1] + R[2, 2] - 1))
    axis_angle = sp.Matrix(4, 1, [ang,
                            1 / (2 * sp.sin(ang)) * (R[2, 1] - R[1, 2]),
                            1 / (2 * sp.sin(ang)) * (R[0, 2] - R[2, 0]),
                            1 / (2 * sp.sin(ang)) * (R[1, 0] - R[0, 1])])
    return axis_angle

def T2axis(T):
    axis_pose = sp.Matrix(7, 1, [0]*7)
    axis_pose[0:3, 0] = T[0:3, 3]
    axis_pose[3:7, 0] = R2axis(T[0:3, 0:3])
    return axis_pose